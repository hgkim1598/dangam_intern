          ...
          <div :class="message.from == 'user' ? 'messageFromUser' : 'messageFromChatGpt'">
            <div :class="message.from == 'user' ? 'userMessageWrapper' : 'chatGptMessageWrapper'">
              <div v-if="message.from == 'user'" style="display: flex; justify-content: flex-end;">
                <img src="@/assets/profile_img.jpg" alt="User Image" style="max-width: 40px; max-height: 40px; border-radius: 50%;">
              </div>
              <img v-else src="@/assets/newicon_1.png" alt="Chatbot Image" style="max-width:100px; max-height: 100px;">
...
export default {
  ...
  mounted() {
    this.chatboxElement = this.$refs.messageBox;
  },
  methods: {
    async sendMessage() {
  ...
  try {
    const response = await axios.post('http://192.168.0.149:8000/chat', requestBody);
