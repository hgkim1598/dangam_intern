<template>
  ...
  export default {
    ...
    methods: {
      async fetchData(id) {
        try {
          const response = await axios.get(`api/saying/${id}`);
      ...
      async fetchCategories() {
        try {
          const response = await axios.get('api/category/?select_category=saying');
      ...
      async submitForm() {
        ...
        try {
          if (this.isUpdateMode) {
            await axios.put(`api/saying/edit/${this.editId}`, data);
            console.log('데이터 수정 성공');
          } else {
            await axios.post('api/saying/new', data);
