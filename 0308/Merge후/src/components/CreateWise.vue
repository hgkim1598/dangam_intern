...
export default {
  ...
  methods: {
    async fetchData(id) {
      try {
        const response = await axios.get(` https://quotes.api.thegam.io/saying/${id}`);
    ...
    async fetchCategories() {
      try {
        const response = await axios.get(' https://quotes.api.thegam.io/category/?select_category=saying');
    ...
    async submitForm() {
      ...
      try {
        if (this.isUpdateMode) {
          await axios.put(` https://quotes.api.thegam.io/saying/edit/${this.editId}`, data);
          console.log('데이터 수정 성공');
        } else {
          await axios.post(' https://quotes.api.thegam.io/saying/new', data);
